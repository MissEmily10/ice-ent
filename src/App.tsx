...
type Position = { id: string; title: string; group: string; description: string; isChannel: boolean; isOpen: boolean; applicants: number; accent: string }
...
function App() {
  ...
  const [catalogPositions, setCatalogPositions] = useState(positions)
  const [selected, setSelected] = useState<Position | null>(null)
  const [showForm, setShowForm] = useState(false)
  const [submitted, setSubmitted] = useState(false)
  const [language, setLanguage] = useState<'RU' | 'EN'>('RU')
  useEffect(() => { if (api.enabled) api.catalog(scope, onlyOpen).then((items) => setCatalogPositions(items.map((item, index) => ({ id: item.id, title: item.title, group: item.group.title, description: item.description ?? '', isChannel: item.group.is_channel_level, isOpen: item.is_open, applicants: 0, accent: positions[index % positions.length].accent })))).catch(() => undefined) }, [scope, onlyOpen])
  const visiblePositions = catalogPositions.filter((position) => (scope === 'all' || (scope === 'channel' ? position.isChannel : !position.isChannel)) && (!onlyOpen || position.isOpen))
...
function ApplicationForm({ position, submitted, setSubmitted }: { position: Position; submitted: boolean; setSubmitted: (value: boolean) => void }) { if (submitted) return <div className="success-state"><div className="success-icon"><Check size={28} /></div><p className="eyebrow">ЗАЯВКА ОТПРАВЛЕНА</p><h1>Спасибо, что<br /><i>решились.</i></h1><p>Мы посмотрим ответы и вернёмся к вам в течение 5 рабочих дней.</p><button className="secondary-button" onClick={() => setSubmitted(false)}>Изменить заявку</button></div>; return <div className="form-layout"><div><p className="eyebrow">ЗАЯВКА · {position.title.toUpperCase()}</p><h1>Расскажите<br /><i>о себе.</i></h1><p className="intro-copy">Не нужно быть идеальным. Нам важно понять, что вам интересно.</p></div><form className="application-form" onSubmit={async (event) => { event.preventDefault(); if (api.enabled) await api.submitApplication(position.id, []); localStorage.setItem('ice-submission', JSON.stringify({ title: position.title, group: position.group, date: new Date().toLocaleDateString('ru-RU'), status: 'pending' })); setSubmitted(true) }}><label>Как к вам обращаться?<input required placeholder="Ваше имя" /></label><label>Почему вам интересна эта роль?<textarea required placeholder="Пара предложений о вашей мотивации" rows={4} /></label><label>Есть что добавить? <span className="optional">необязательно</span><textarea placeholder="Ссылки, опыт, идеи" rows={3} /></label><label className="file-label"><span>Резюме или портфолио <span className="optional">необязательно</span></span><span className="file-drop"><Paperclip size={17} /> Прикрепить файл<input type="file" /></span></label><button className="primary-button" type="submit">Отправить заявку <Send size={16} /></button></form></div> }

function Applications() { const [items, setItems] = useState(submissions); useEffect(() => { if (api.enabled) api.applications().then((remote) => setItems(remote.map((item) => ({ title: item.position, group: item.group, date: new Date(item.created_at).toLocaleDateString('ru-RU'), status: item.status })))).catch(() => undefined); const saved = localStorage.getItem('ice-submission'); if (saved) setItems((current) => [JSON.parse(saved), ...current.filter((item) => item.title !== JSON.parse(saved).title)]) }, []); return <div className="page inner-page"><div className="section-heading"><div><p className="eyebrow">ЛИЧНЫЙ РАЗДЕЛ</p><h1>Мои заявки.</h1></div><span className="heading-count">{String(items.length).padStart(2, '0')} всего</span></div><div className="application-list">{items.map((item) => <div className="application-row" key={`${item.title}-${item.date}`}><div className="application-symbol"><FileText size={21} /></div><div className="application-info"><h3>{item.title}</h3><span>{item.group} · {item.date}</span></div><span className={`status ${item.status}`}>{statusLabel[item.status]}</span><ChevronRight size={18} className="row-arrow" /></div>)}</div></div> }
